function [G,subset] = ICD(X,kernel,m,precision)
...
for i = 1:m	% column index
	...
	if i == 1
		...
		d(i:n) = ones(length(i:n),1)';
	else
		% update the diagonal of the residual kernel matrix
		d(i:n) = ones(length(i:n),1)' - sum(G(i:n,1:i-1).^2,2)';
	end
	dtrace = sum(d(i:n));
	...
	G(i+1:n,i) = (kernel.build(X(perm(i),:),X(perm(i+1:n),:))'...
